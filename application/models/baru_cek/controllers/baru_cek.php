...
if($data['jenis']==2) {  // request bpkb 


	$wsdl = "http://service.bpkb.net/dv/bpkbsvc.dll/wsdl/IBpkbDataService"; 
    $client = new SoapClient($wsdl);

    // show_array($client); exit;
    $param = array("UserName"=>"mandiri",
    			   "Password"=>"mandiri");

    $val = $client->SignIn("mandiri","mandiri");
    

    // $value = $client->SignIn0Response;
    ...
    // echo "value $value";
   	...
   	if($val['return'] == 1 ) {
   		...
   		$dataranmor = $client->GetBpkbData($sid,$data['no_rangka'],"spNoBpkb");

   		show_array($dataranmor); 
   		// $ran_data = (array) $dataranmor->Data;

   		$data['no_rangka'] = $dataranmor['Data']->NoRangka;
